...
function initProjectGallery(vid_img,projectId, mediaItems, elements) {
    // State management
    let currentIndex = 0;
        ...
        elements.mainPreview.innerHTML = `
            <div class="w-full h-full flex items-center justify-center text-gray-500 dark:text-gray-400">
                <div class="text-center">
                    ...
                    <p class="mt-2">No media available for this project</p>
                </div>
            </div>
        `;
    }
    ...
    function createMediaElement(item, vid_img, isMainPreview = false) {
        ...
        fallback.innerHTML = `
            <div class="text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    function renderThumbnails(vid_img) {
        ...
        mediaItems.forEach((item, index) => {
            ...
            thumb.className = `flex-shrink-0 relative cursor-pointer transition-all duration-200 ease-in-out ${
                index === currentIndex ? 'ring-2 ring-blue-500 scale-105 rounded-md' : 'hover:ring-1 hover:ring-blue-300 rounded-md'
            }`;
            ...
            if (item.type === 'image') {
                ...
                previewVideo.className = 'absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition-opacity duration-300';
    ...
    function updateGallery() {
        ...
        document.querySelectorAll(`#media-thumbnails-${projectId} button, #fullscreen-thumbnails-${projectId} button`).forEach((btn, idx) => {
            if (idx === currentIndex) {
    ...
    function renderFullscreenPreview(item) {
        ...
        document.getElementById(`fullscreen-title-${projectId}`).textContent = item.title || '';
        document.getElementById(`fullscreen-about-${projectId}`).textContent = item.about || '';
        
        // Update media counter
        document.getElementById(`current-index-${projectId}`).textContent = currentIndex + 1;
        document.getElementById(`total-media-${projectId}`).textContent = mediaItems.length;
        ...
        if (item.type === 'image') {
            ...
            const videoControls = document.getElementById(`video-controls-${projectId}`);
            videoControls.classList.remove('hidden');
            
            // Set up video controls
            const playPauseBtn = document.getElementById(`play-pause-${projectId}`);
            const progressBar = document.getElementById(`progress-bar-${projectId}`);
            const currentTimeEl = document.getElementById(`current-time-${projectId}`);
            const durationEl = document.getElementById(`duration-${projectId}`);
            const muteBtn = document.getElementById(`mute-${projectId}`);
            const fullscreenBtn = document.getElementById(`fullscreen-video-${projectId}`);
    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll(`#media-thumbnails-${projectId} button, #fullscreen-thumbnails-${projectId} button`).forEach(btn => {
